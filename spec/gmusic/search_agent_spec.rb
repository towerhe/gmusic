# encoding: UTF-8
describe Gmusic::SearchAgent do
  ...
  describe '.search' do
    ...
    subject { Gmusic::SearchAgent.search(query) }

    it 'returns a result object' do
      should be_a Gmusic::Result
    end
  ...
  describe 'private class methods' do
    let(:agent) { Gmusic::SearchAgent.send(:agent) }
    ...
    describe '.extract_info_from' do
      describe 'when not found' do
        ...
        it 'raises NotFound' do
          expect do
            Gmusic::SearchAgent.send(:extract_info_from, @page)
          end.to raise_error('Gmusic::NotFound')
        end
      end
      ...
      describe 'when found' do
        ...
        subject { Gmusic::SearchAgent.send(:extract_info_from, @page) }
    ...
    describe '.collect_links_from' do
      ...
      subject { Gmusic::SearchAgent.send(:collect_links_from, @page) }
    ...
    describe '.format' do
      it 'replaces blank space to %20' do
        Gmusic::SearchAgent.send(:format, ' word1  word2   ').should eq 'word1%20word2'
      end
    end
    ...
    describe '.format_url' do
      it 'formats the base url with the given hash' do
        ...
        Gmusic::SearchAgent.send(:format_url, base_url, hash).should eq url
      end
    end
  end
end
